...
public class WsprDecoder {
    ...
    private static final String TAG = "WsprDecoder";

    /** Minimum audio frame length required for a complete WSPR transmission. */
    private static final double MIN_FRAME_SECONDS = 110.0;
        ...
        try {
            ...
            if (buffer.getDurationSeconds() < MIN_FRAME_SECONDS) {
                System.err.println(TAG + ": buffer duration "
                        ...
                        + "s is below the WSPR minimum of " + MIN_FRAME_SECONDS + "s; skipping");
                return results;
            }

            // Read only the last 120 seconds — avoids copying the entire accumulated buffer
            ...
            String decodedText = "[WSPR FRAME DETECTED] Peak: " + String.format("%.1f", peakHz) + " Hz";
